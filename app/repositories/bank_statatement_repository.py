...
class BankStatementRepository():
    ...
    def insert(self, balance, money, total, account_id, date=date.today()):
        try:
            with DbConnectionHandler() as db:
                bank_statement = Bank(balance=balance, money=money, total=total, date=date, account_id=account_id)
    ...
    def select_all_by_id_offset(self, id, offset):
        try:
            with DbConnectionHandler() as db:
                ...
                return db.session.query(Bank).filter(Bank.account_id == id).order_by(Bank.date.desc()).offset(offset=offset).limit(10)
        except Exception:
            return Exception.with_traceback
